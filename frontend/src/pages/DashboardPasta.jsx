...
export default function DashboardPasta() {
  ...
  const [distribuicao, setDistribuicao] = useState({ nivel: "atividade", itens: [] });
  ...
  useEffect(() => {
    let cancelado = false;
    Promise.all([
      pastasApi.buscar(pastaId),
      dashboardApi.kpis({ pastaId }),
      dashboardApi.heatmap({ pastaId }),
      dashboardApi.distribuicao({ pastaId }),
      dashboardApi.evolucao({ pastaId }),
      dashboardApi.porHora({ pastaId }),
      dashboardApi.porDiaSemana({ pastaId }),
      dashboardApi.porModo({ pastaId }),
    ])
      .then(
        ([
          pastaRes,
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[2fr_1fr] lg:items-start">
        ...
        <Cartao titulo="Atividades da pasta">
          {distribuicao.itens.length > 0 ? (
            <DonutDistribuicao
              itens={distribuicao.itens}
              rotaParaItem={(item) =>
                item.atividadeId
                  ? `/atividades/${item.atividadeId}/dashboard`
                  : null
              }
            />
          ) : (
