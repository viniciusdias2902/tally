...
export default function CardAtividade({ atividade, nomePasta, onEditar, onArquivar, onDeletar }) {
  ...
  return (
    <div className="flex flex-col rounded-xl border border-black/[0.06] dark:border-white/[0.08] bg-bg-elevated p-5 shadow-sm transition-all duration-150 hover:shadow-md">
      ...
      <div className="flex-1 mb-4 min-w-0">
        ...
        {nomePasta && (
          <span className="inline-flex items-center gap-1 mt-2 px-2 py-0.5 rounded-md bg-bg-secondary text-text-secondary text-xs font-medium max-w-full">
            <FolderIcon />
            <span className="truncate">{nomePasta}</span>
          </span>
        )}
      </div>
      ...
      <Link
        ...
        className="inline-flex items-center justify-center gap-2 rounded-xl font-medium transition-all duration-150 ease-in-out cursor-pointer bg-accent text-accent-foreground shadow-sm hover:bg-accent-hover px-4 py-2 text-sm w-full"
      >
        <PlayIcon /> Registrar
      </Link>
...
function FolderIcon() {
  return (
    <svg className="w-3 h-3 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75} aria-hidden="true">
