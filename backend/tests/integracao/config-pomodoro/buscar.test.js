...
describe("GET /atividades/:atividadeId/config-pomodoro", () => {
  let accessToken;
  let atividadeId;
  ...
  it("deve retornar a configuração existente com 200", async () => {
    await request(app)
    ...
    const res = await request(app)
      .get(`/atividades/${atividadeId}/config-pomodoro`)
      .set("Authorization", `Bearer ${accessToken}`);
  ...
  it("deve retornar 404 quando não há configuração", async () => {
    const res = await request(app)
      .get(`/atividades/${atividadeId}/config-pomodoro`)
      .set("Authorization", `Bearer ${accessToken}`);
  ...
  it("deve retornar 404 para atividade inexistente", async () => {
    ...
    const res = await request(app)
      .get(`/atividades/${uuidInexistente}/config-pomodoro`)
      .set("Authorization", `Bearer ${accessToken}`);
